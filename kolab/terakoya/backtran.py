...
class CExpr(object):  # Code Expression
    ...
    def match(self, model) -> NExpr:
        ...
        if name in model.rules:
            ...
            if len(self.params) > 0 and isinstance(self, CApp):  
...
class Reader(ParseTreeVisitor):
    ...
    def acceptNDocument(self, tree):
        s = str(tree).split('->')[0].strip()
        nexpr, self.symbols = tokibi.parse2(s, self.synonyms)
        # nexpr = tokibi.test_parse(nexpr.src())
        return nexpr
...
class KotohaModel(object):
    ...
    def translate(self, expression, suffix='。'):
        try:
            ...
            #     return code, main
            # pred2 = tokibi.parse(pred.src())
            # print('@debug', pred.src(), pred2.src())
            return code, pred.src()
