...
x &= y # xをyとの/論理積にする
x ^= y # xをyとの/排他論理和にする
x <<= y # xをyビット左に/シフトする | xをyビット/左シフトする
x >>= y # xをyビット右に/シフトする | xをyビット/右シフトする
...
hasattr(x, attrname)  # x がattrnameを/持つかどうか -> bool
...
str.find(sub)  # str内で、subが見つかる -> int(位置)
str.find(sub)  # str内で、subが見つかる -> int(位置)
str.find(sub)  # str内で、subが最初に/見つかる -> int(位置)
str.find(sub, start)  # str内をstartから探したとき、subが最初に/見つかる -> int(位置)
str.find(sub, start, end)
# str内をstartからendまで探したとき、subが最初に/見つかる -> int(位置)

str.find(sub) >= 0  # str内で/subが見つかるかどうか -> bool
str.find(sub) == -1  # str内で/subが見つからないかどうか -> bool

str.index(sub)  # str内で/subが最初に/見つかる -> int(位置)
str.index(sub, start)  # str内をstartから探したとき、subが最初に/見つかる -> int(位置)
str.index(sub, start, end)
# str内をstartからendまで探したとき、subが最初に_/見つかる -> int(位置)

str.rfind(sub)  # str内で/subが最後に/見つかる -> int(位置)
str.rfind(sub, start)  # str内をstartから探したとき、subが最後に/見つかる -> int(位置)
str.rfind(sub, start, end)
# str内をstartからendまで探したとき、subが最後に/見つかる -> int(位置)

str.rindex(sub)  # str内でsubが最後に/見つかる -> int(位置)
str.rindex(sub, start)  # str内をstartから探したとき、/subが最後に/見つかる -> int(位置)
